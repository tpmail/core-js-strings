...
function countVowels(str) {
  ...
  let count = 0;
  for (let i = 0; i < str.length; i += 1) {
    if (['a', 'e', 'i', 'o', 'u', 'y'].includes(str[i].toLocaleLowerCase())) {
      count += 1;
    }
  }
  return count;
...
function isPalindrome(str) {
  const clearString = str.toLowerCase().replace(/[^a-zA-Z]/g, '');
  for (let l = 0, r = clearString.length - 1; l < r; l += 1, r -= 1) {
...
function extractNameFromTemplate(value) {
  const index = value.indexOf('Hello, ');
  return index !== -1 ? value.slice(index + 7, -1) : '';
}
  ...
  for (let i = 0; i < str.length; i += 1) {
    ...
    if (str[i].charCodeAt() >= 65 && str[i].charCodeAt() <= 90) {
      if (str[i].charCodeAt() <= 77) {
        result += String.fromCodePoint(str[i].charCodeAt() + 13);
      } else {
        result += String.fromCodePoint(str[i].charCodeAt() - 13);
      }
    } else if (str[i].charCodeAt() >= 97 && str[i].charCodeAt() <= 122) {
